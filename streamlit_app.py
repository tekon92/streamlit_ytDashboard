...
def style_positive(v, props=''):
    """ style postive value in dataframe"""
    try:
        return props if v > 0 else None
    except:
        pass
...
def audience_simple(country):
    """ show top represented countried"""
    if country == 'US':
        return 'USA'
    elif country == 'IN':
        return 'India'
...
def load_data():
    ...
    df_time['time'] = pd.to_datetime(df_time['Date'], format='mixed')

    # st.write(df_agg)
    return df_agg, df_agg_sub, df_comments, df_time

# create dataframe from the function
df_agg, df_agg_sub, df_comments, df_time = load_data()

# additional data engineering for aggregated data
df_agg_diff = df_agg.copy()
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months =12)
median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].select_dtypes(include=['float64', 'int']).median()
...
df_time_diff['days_published'] = (pd.to_datetime(df_time_diff['Date'], format='mixed') - df_time_diff['Video publish time']).dt.days
...
view_cumulative = view_days.loc[:, ['days_published', 'median_views', '80pct_views', '20pct_views']]
view_cumulative.loc[:, ['median_views', '80pct_views', '20pct_views']] = view_cumulative.loc[:, ['median_views', '80pct_views', '20pct_views']].cumsum()
# st.write(view_cumulative)
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_medians6mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_6mo].select_dtypes(include=['float64', 'int']).median()
    metric_medians12mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo].select_dtypes(include=['float64', 'int']).median()

    # st.write(metric_medians6mo)
    ...
    df_agg_numeric_lst = df_agg_diff_final.select_dtypes(include=['float64', 'int']).median().index.to_list()
    # st.write(df_agg_diff_final)
    df_to_pct = {}
...
if add_sidebar == 'Individual Video Analysis':
    ...
    fig2.add_trace(go.Scatter(
        x=view_cumulative['days_published'], 
        y=view_cumulative['20pct_views'],
        mode='lines',
    ...
    fig2.add_trace(go.Scatter(
        x=view_cumulative['days_published'],
        y=view_cumulative['median_views'],
        mode='lines',
